...
class Terrain {
    ...
    private static int subdivisions = 40; // relatively large value.. will be lowered for final terrain
    ...
    public void draw(GL2 gl, GLU glu, GLUT glut) {
        ...
        for (int subsY = 0; subsY <= subdivisions; subsY++) {
            ...
            for (int subsX = 0; subsX <= subdivisions; subsX++) {
                gl.glPushMatrix();
                Vector pos = TerrainUtility.getPointSurface(subsX/(float)subdivisions, subsY/(float)subdivisions, TerrainUtility.controlPoints);
        ...
        for (int i = 0; i < 16; i++) {
            ...
            Vector pos = TerrainUtility.controlPoints[i];
            gl.glPushMatrix();
            gl.glColor3f(1,0,0);
            gl.glTranslatef((float)pos.x, (float)pos.y, (float)pos.z);
        ...
        points = TerrainUtility.controlPoints;
